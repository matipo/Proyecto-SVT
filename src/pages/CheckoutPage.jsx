...
function PaymentForm({ reservationId, reservationItems, event, total }) {
  ...
  //Maneja el envio del formulario de pago al backend
  const handleCheckoutSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        //la reserva ya no es valida
        if (
          response.status === 400 &&
          data.detail &&
          typeof data.detail === "string" &&
          data.detail.includes("not active")
        ) {
          errorMessage =
            "Tu reserva ha expirado o ya fue confirmada. Por favor, inténtalo de nuevo desde la página del evento.";
          // Eliminamos la reserva guardada en sessionStorage
          ...
          if (typeof data.detail === "object") {
            if (
              ...
              errorMessage = `Error en el campo ${data.detail[0].loc.join(
                " > "
              )}: ${data.detail[0].msg}`;
      ...
      // Si la compra fue exitosa, limpiamos el registro en sessionStorage
      const reservationKey = `reservation_${event._id}_${reservationItems[0].type}_${reservationItems[0].quantity}`;
      sessionStorage.removeItem(reservationKey);

      // Navegamos a la página de compra realizada, pasando los datos de la compra
      navigate(`/event/purchase-success`, {
        state: { purchaseData: data, eventData: event },
      });
    } catch (err) {
...
//Valida una reserva y muestra el formulario de pago si es válida.
export default function CheckoutPage() {
    ...
    const validateReservation = async () => {
      try {
        const response = await fetch(
          `${url2}/${passedReservation.reservation_id}`
        );
        //reserva no encontrada o expirada
        if (response.status === 404) {
          throw new Error(
            "Tu reserva ha expirado o no fue encontrada. Por favor, inténtalo de nuevo."
          );
        }
        //El mensajito de error que salta cuando tu reserva expiro
        if (!response.ok) {
          const data = await response.json();
          ...
          if (
            ...
            throw new Error(
              "Tu reserva ha expirado. Por favor, inténtalo de nuevo."
            );
          }
        ...
        //la reserva es inválida, etonce la borramos de sessionStorage
        if (
          event?._id &&
          passedReservation?.items?.[0]?.type &&
          passedReservation?.items?.[0]?.quantity
        ) {
          const reservationKey = `reservation_${event._id}_${passedReservation.items[0].type}_${passedReservation.items[0].quantity}`;
          sessionStorage.removeItem(reservationKey);
        }
      } finally {
        setIsLoading(false); // Quitamos el loading
      }
    };
  ...
  //reserva valida, mostramos el formulario y el resumen
  const subtotal = validReservation.total_price;
  const iva = subtotal * 0.19;
  const total = subtotal + iva;
  // RENDER PRINCIPAL a la pagina
  return (
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 sticky top-8">
            ...
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-600">
                {validReservation.items[0].quantity} x{" "}
                {validReservation.items[0].type}
              </p>
            </div>

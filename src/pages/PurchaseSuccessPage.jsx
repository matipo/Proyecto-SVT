    ...
    return (
        <div className="p-4 md:p-8 max-w-3xl mx-auto my-10">
            <div className="bg-white rounded-xl shadow-2xl p-6 md:p-10 text-center">
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">¡Tu compra ha sido confirmada!</h1>
                ...
                <div className="bg-gray-50 rounded-lg p-6 text-left space-y-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-700 font-mono text-sm">{purchaseId || "---"}</p>
                    </div>
                        ...
                        {tickets && tickets.length > 0 ? (
                            <ul className="space-y-3">
                                {tickets.map((ticket, index) => (
                                    <li key={index} className="p-4 bg-white border border-gray-200 rounded-lg flex justify-between items-center">
                                        <div>
                                            <span className="font-semibold text-gray-800">{ticket.type || "Ticket"}</span>
                                            <p className="text-sm text-gray-500">Ticket #{index + 1}</p>
                                        </div>
                                        <div className="text-right">
                                            <span className="block font-semibold text-sm">CÓDIGO:</span>
                                            <span className="font-mono text-blue-600 font-bold">{ticket.code || "---"}</span>
                                        </div>
                                    </li>
                                ))}
                            </ul>
                        ) : (
                            <p className="text-gray-600 text-center">No se encontraron detalles de los tickets.</p>
                        )}
                    </div>
                </div>
